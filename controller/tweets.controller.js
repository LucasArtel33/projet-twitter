const { getTweet, getTweets, createTweet, deleteTweet, updateTweet, getCurrentUserTweetWithFollowing } = require ('../queries/tweet.queries');

exports.tweetList = async (req, res, next) => {
  try {
    const tweets = await getCurrentUserTweetWithFollowing(req.user) ;
    res.render('tweets/tweet' , { tweet: {}, tweets, isAuthenticated: req.isAuthenticated(), currentUser: req.user, user: req.user  });
  } catch (e) {
    next(e);
  }
}
...
exports.tweetDelete = async (req, res, next) => {
  try {
    ...
    const tweets = await getCurrentUserTweetWithFollowing(req.user);
    res.render('tweets/tweet-list', { tweets, currentUser: req.user })
  } catch (e) {
    next(e)
  }
}
